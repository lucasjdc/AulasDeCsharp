    ...
    internal  class ByteBankAtendimento
    {
        ...
        private void ExportarContasXML()
        {
            ...
            else
            {
                ...
                try
                {
                    FileStream fs = new FileStream(@"c:\tmp\export\contas.xml", FileMode.Create);
                    using (StreamWriter streamwriter = new StreamWriter(fs))
                    {
                        new XmlSerializer(typeof(List<ContaCorrente>)).Serialize(streamwriter, _listaDeContas);
                    }
        ...
        private void ExportarContas()
        {
            ...
            if (_listaDeContas.Count <= 0)
            {
                Console.WriteLine("...Não exite dados para exportação...");
                Console.ReadKey();
            }
            else
            {
